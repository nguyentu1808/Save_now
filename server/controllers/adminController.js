...
const getWarehouses = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching warehouses" });
  }
};
...
const updateWarehouse = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows > 0) {
      return res
        .status(200)
        .json({ success: true, message: "Warehouse updated successfully" });
    } else {
      return res
        .status(404)
...
const deleteWarehouse = async (req, res) => {
  ...
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ success: false, message: "ID kho không hợp lệ" });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Kho đã được xóa thành công" });
  } catch (err) {
    console.error(err);
...
const addWarehouse = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error adding warehouse" });
  }
};

// Thêm nhân viên
const addEmployee = async (req, res) => {
  const { user_name, email, password, full_name, role, warehouse_id } =
    req.body;
  if (
    !user_name ||
    !email ||
