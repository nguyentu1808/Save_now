...
const AdminWInfoRequest = () => {
  ...
  useEffect(() => {
    const fetchItems = async requestId => {
      try {
        const response = await fetch(`${apiUrl}/adRequest/items/request/${requestId}`, {
          method: 'GET',
          headers: {
  ...
  const fetchArticle = async requestId => {
    try {
      const response = await fetch(`${apiUrl}/adRequest/article/${requestId}`, {
        method: 'GET',
        headers: {
...
export default AdminWInfoRequest;
