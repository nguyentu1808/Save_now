...
const AdminWRescueRequestList = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      try {
        const response = await fetch(`${apiUrl}/adRequest/requests?status=${status}`, {
          method: 'GET',
          headers: {
  ...
  const handleApprove = async id => {
    try {
      const response = await fetch(`${apiUrl}/adRequest/requests/${id}/status`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  const handleRequestClick = requestId => {
    navigate(`/adminW-request/inforequest/${requestId}`);
  };
    ...
    <Layout>
      <main style={styles.main}>
        <h2 style={styles.pageTitle}>
          ...
          <span
            style={activeTab === 'approvals' ? styles.activeTab : styles.inactiveTab}
            ...
            rescueRequests.map(request => (
              ...
              >
                ...
                <div style={styles.requestContent}>
                  ...
                  <p style={styles.requestDescription}>Mô tả: {request.description}</p>
                </div>
              </div>
            ))}
              ...
              <div key={request.id} style={styles.requestItem}>
                ...
                <div style={styles.requestContent}>
                  ...
                  <div style={styles.actionButtons}>
                    <button style={styles.rejectButton} onClick={() => handleReject(request.id)}>
                      <FaTimes />
                    </button>
                    <button style={styles.approveButton} onClick={() => handleApprove(request.id)}>
                      <FaCheck />
                    </button>
                  </div>
                </div>
        ...
        {activeTab === 'requests' && (
          <button style={styles.createRequestButton} onClick={() => setIsAddRequestVisible(true)}>
            Tạo yêu cầu
          </button>
        )}
        {isAddRequestVisible && (
          <AddRequest onClose={() => setIsAddRequestVisible(false)} onAddRequest={addNewRequest} />
        )}
      </main>
    </Layout>
  );
...
export default AdminWRescueRequestList;
