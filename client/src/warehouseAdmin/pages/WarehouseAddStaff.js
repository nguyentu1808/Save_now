...
export default function AddUserModal({ onClose, onAdd }) {
  ...
  const handleAddUser = async () => {
    const { user_name, password, email, full_name } = newUser;
    ...
    try {
      const response = await fetch('http://localhost:5001/adminW/addEmployee', {
        ...
        body: JSON.stringify({ user_name, password, email, full_name }),
      });
      ...
      if (response.ok) {
        ...
        onAdd({ user_name, email, full_name }); // Call the onAdd callback to update the list of employees
