...
export default function AddItemModal({ onClose, onSave, editingItem, categories, warehouse_id }) {
  ...
  const handleSave = async () => {
    ...
    const formattedDate = item.expiration_date ? mysqlDateFormat(item.expiration_date) : null;
    if (editingItem) {
      try {
        // console.log(item);
        // Gửi yêu cầu cập nhật thông tin nhân viên tới API
        const response = await fetch(`${apiUrl}/adminW/item/${editingItem.id}`, {
          method: 'PUT',
          headers: {
      ...
      try {
        console.log(item);
        const response = await fetch(`${apiUrl}/adminW/addItem`, {
          method: 'POST',
          headers: {
  ...
  return (
    <div style={styles.modal}>
      <div style={styles.modalContent}>
        ...
        />
        <select style={styles.input} name="category" value={item.category} onChange={handleChange}>
